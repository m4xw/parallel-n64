...
#if 0
	rd &= 15;
	uint32_t res = *RSP::cpu.get_state().cp0.cr[rd];
	...
	if (rd == CP0_REGISTER_SP_STATUS)
	{
		...
		if (RSP::MFC0_count[rt] >= RSP::SP_STATUS_TIMEOUT)
		{
			*RSP::rsp.SP_STATUS_REG |= SP_STATUS_HALT;
			lightrec_set_exit_flags(rec, LIGHTREC_EXIT_MISC);
		}
	}
...
static inline void rsp_status_write(lightrec_state *rec, uint32_t rt)
{
	...
	*rsp->cp0.cr[CP0_REGISTER_SP_STATUS] = status;
	if ((*rsp->cp0.irq & 1) || (status & SP_STATUS_HALT))
	{
		fprintf(stderr, "HALT!\n");
		lightrec_set_exit_flags(rec, LIGHTREC_EXIT_MISC);
		lightrec_set_target_cycle_count(rec, lightrec_current_cycle_count(rec));
	}
}

#ifdef PARALLEL_INTEGRATION
static void rsp_dma_read(lightrec_state *rec)
{
	auto *rsp = &RSP::cpu.get_state();
	...
	*rsp->cp0.cr[CP0_REGISTER_DMA_DRAM] &= ~0x7;
	...
	do
	{
		...
		do
		{
			...
			if (dest_addr & 0x1000)
			{
				...
				invalidate_imem = true;
				//lightrec_invalidate(rec, dest_addr, 4);
			}
			else
	...
	if (invalidate_imem)
	{
		lightrec_invalidate_all(rec);
		lightrec_set_exit_flags(rec, LIGHTREC_EXIT_MISC);
		lightrec_set_target_cycle_count(rec, lightrec_current_cycle_count(rec));
		fprintf(stderr, "INVALIDATE ALL!\n");
	}
...
void RSP_MTC0(lightrec_state *rec, u32 val, u32 reg)
{
	auto *rsp = &RSP::cpu.get_state();

	switch (static_cast<CP0Registers>(reg & 15))
	{
		...
		case CP0_REGISTER_SP_STATUS:
			return rsp_status_write(rec, val);
